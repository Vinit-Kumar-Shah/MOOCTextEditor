...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		{
			if(i == charArray.length - 1 && Character.toLowerCase(charArray[i]) == 'e' && ifSyllable && numSyllables > 0)
			{
				numSyllables--;
			}
			
			if(ifSyllable && vowels.indexOf(Character.toLowerCase(i))>=0)
			{
				ifSyllable = false;
				numSyllables++;
			}
	...
	public double getFleschScore()
	{
		...
		/*double FleschScore = 206.835 - (double)1.015*(getNumWords()/getNumSentences()) - (double)84.6*(getNumSyllables()/getNumWords()); 
	    return FleschScore;*/
		double wordCount = (double) getNumWords();
	    double FlechNumber = 206.835 - (1.015 * ((wordCount) / getNumSentences())) - (84.6 * (((double) getNumSyllables()) / wordCount));
